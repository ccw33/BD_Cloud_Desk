...
<script>
  ...
  export default {
    ...
    created() {
      ...
      // let is_connecting_it = window.setInterval(is_connecting, 3000)
      ...
      let start_ws = () => {
        // 首先先获取服务器的ip和端口,获取完以后就与服务器进行长连接心跳
        this.$ajax.get('/api/other_operation/get_vm_server', {}).then((resp) => {
          let data = resp.data;
          let ws = new WebSocket(`wss://${data.vm_server_host}:${data.vm_server_port}/websocket`, "base64");
          ...
          ws.onclose = function (evt) {
            ...
            console.log("Unkown server host and port, so connect fail 无法获取服务器的ip和端口,所以websocket连接失败");
        });
      }
