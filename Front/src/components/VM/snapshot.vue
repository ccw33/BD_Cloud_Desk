    ...
    methods: {
      create_snapshot() {
        ...
        vm.$ajax.get('/api/operation_system/snapshot_operation/snapshot', {
          ...
          //显示创建成功
          vm.$root.alert.success.message = '创建快照成功'
          ...
          if (error && error.response && error.response.status == 401)
            return
          vm.$root.alert.fail.message = `创建快照失败：${error.response.data.content}`
      ...
      delete_snapshot() {
        ...
        if(vm.$store.state.focus_node.snapshot_id == vm.$store.getters.current_snapshot.snapshot_id){
          vm.$root.alert.fail.message = `不能删除正在使用的快照`
          vm.$root.alert.fail.is_show = true;
          return
        }
        vm.$ajax.get('/api/operation_system/snapshot_operation/snapshot_delete', {
          ...
          if (error && error.response && error.response.status == 401)
            return
          vm.$root.alert.fail.message = `删除快照失败：${error.response.data.content}`
          vm.$root.alert.fail.is_show = true;
        }).finally();
      },
      go_to_snapshot() {
        ...
        vm.$ajax.get('/api/operation_system/snapshot_operation/rollback_snapshot', {
          ...
          if (error && error.response && error.response.status == 401)
            return
          vm.$root.alert.fail.message = `转到快照失败：${error.response.data.content}`
          vm.$root.alert.fail.is_show = true;
        }).finally();
      }
    },
