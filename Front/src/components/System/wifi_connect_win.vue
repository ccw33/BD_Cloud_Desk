<template>
  <div class="container-fluid">
    ...
    <article>
      <div class="title">
        <div class="icon"></div>
        <div class="text">wifi网络 {{$attrs.prop_wifi.name}} 需要{{$attrs.prop_wifi.key_type}}密码</div>
      </div>
      <form action="">
        <section class="password">
          <label for="password">密码：</label>
          <input :type="password_type" v-model="$attrs.prop_wifi.password" class="form-control" id="password"
        ...
        <section class="checkboxs">
          ...
          <label>
            <input type="checkbox" v-model="$attrs.prop_wifi.remember_pwd"> 记住该网络
          </label>
        </section>
      </form>
    </article>
...
<script>
  ...
  export default {
    ...
    methods: {
      submit() {
        const vm = this;
        // vm.$attrs.prop_wifi.password = $('#password').val();
        vm.$ajax.post(
          `${vm.host}/api/operation_system/connect_wifi`,
          vm.$qs.stringify(vm.$attrs.prop_wifi),
        ).then(resp => {
          vm.$emit('cancel');
          vm.$ajax.get(
            `/api/operation_system/is_access_point_actived`,
            {
              params: {
                ssid: vm.$attrs.prop_wifi.name,
                ap_mac: vm.$attrs.prop_wifi.ap_mac,
              }
            },
          ).then(resp => {
            vm.$root.alert.success.message = `连接wifi${vm.$attrs.prop_wifi.name}成功`;
            vm.$root.alert.success.is_show = true;
          }).catch(err => {
            vm.$root.alert.fail.message = `连接wifi${vm.$attrs.prop_wifi.name}失败`;
